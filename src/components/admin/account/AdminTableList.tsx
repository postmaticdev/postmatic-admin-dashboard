...
function AdminAvatar({ name, src, className }: { name: string; src?: string; className: string }) {
  const [hasImageError, setHasImageError] = useState(false);
  const canShowImage = Boolean(src?.trim()) && !hasImageError;

  return (
    <div
      className={`${className} overflow-hidden bg-violet-500/10 text-violet-600 dark:text-violet-400 flex items-center justify-center`}
    >
      {canShowImage ? (
        <img
          src={src}
...
function RoleBadge({ role }: { role: AdminAccount["role"] }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-md ${cfg.color}`}
    >
        ...
        <div className="px-6 pb-6 pt-10 space-y-4">
          ...
          <div className="space-y-2 border-t border-border/60 pt-3">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>
                Bergabung{" "}
                {new Date(admin.joinedAt).toLocaleDateString("id-ID", {
                  day: "numeric",
                  month: "long",
                  year: "numeric",
                })}
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>
                Terakhir aktif{" "}
                {new Date(admin.lastActive).toLocaleDateString("id-ID", {
                  day: "numeric",
                  month: "long",
                  year: "numeric",
                })}
