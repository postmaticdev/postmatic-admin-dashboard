...
interface UserFormViewProps {
  user: UserAccount;
  onSave: (data: Omit<UserAccount, "id" | "postsCount" | "followersCount">, id: string) => void;
...
function DeleteConfirmModal({ userName, onConfirm, onClose }: { userName: string; onConfirm: () => void; onClose: () => void }) {
  const [input, setInput] = useState("");
  const matches = input.trim() === userName.trim();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground">Hapus Pengguna</h3>
            <p className="text-xs text-muted-foreground">Tindakan ini tidak dapat dibatalkan.</p>
          </div>
        </div>
        <p className="text-xs text-muted-foreground leading-relaxed">
          Ketik <span className="font-semibold text-foreground">{userName}</span> untuk mengonfirmasi penghapusan akun ini secara permanen.
        </p>
        <input
          type="text"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={onClose}>
      ...
      >
        ...
        <p className="text-xs text-muted-foreground leading-relaxed">
          Email reset password akan dikirimkan ke <span className="font-semibold text-foreground">{email}</span>. Pengguna perlu mengklik tautan di email untuk membuat password baru.
        </p>
        <div className="flex gap-2 pt-1">
          <button
            type="button"
...
export function UserFormView({ user, onSave, onDelete, onCancel }: UserFormViewProps) {
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-foreground">Edit Pengguna</h1>
          <p className="text-xs text-muted-foreground font-mono">Account / User / {user.fullName}</p>
        </div>
      </div>

      {/* Reset sent notification */}
      {(resetSent || countdown > 0) && (
        <div className="flex items-center justify-between gap-2 px-4 py-3 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-600 dark:text-blue-400 text-sm font-medium">
          <div className="flex items-center gap-2">
            <CheckCircle2 className="h-4 w-4 shrink-0" />
            <span>Email reset password berhasil dikirimkan ke <strong>{form.email}</strong></span>
          </div>
          {countdown > 0 && (
            <span className="text-xs text-muted-foreground bg-blue-500/5 px-2.5 py-1 rounded-lg border border-blue-500/10 whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-6 items-start">
        ...
        <div className="bg-card border border-border/80 rounded-2xl p-6 shadow-sm space-y-5">
          <div className="flex items-center gap-3 pb-2 border-b border-border/60">
            ...
            <div>
              <h2 className="text-sm font-bold text-foreground">Informasi Pengguna</h2>
              <p className="text-xs text-muted-foreground">Edit data profil pengguna</p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card border border-border/80 rounded-2xl p-5 shadow-sm space-y-3">
            ...
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-blue-500/30 bg-blue-500/5 text-blue-600 dark:text-blue-400 text-sm font-semibold hover:bg-blue-500/10 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <Mail className="h-4 w-4" />
              {countdown > 0 ? `Kirim Ulang (${countdown}s)` : "Kirim Reset Password"}
            </button>
          </div>
          ...
          <div className="bg-card border border-red-500/20 rounded-2xl p-5 shadow-sm space-y-3">
            ...
            <p className="text-xs text-muted-foreground">Menghapus akun ini akan menghilangkan semua data pengguna secara permanen.</p>
            <button
              type="button"
      ...
      {showDelete && (
        <DeleteConfirmModal
          ...
          onConfirm={() => { setShowDelete(false); onDelete(user.id); }}
