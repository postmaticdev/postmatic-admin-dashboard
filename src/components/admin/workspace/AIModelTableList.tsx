...
export function AIModelTableList({
  ...
  return (
    <div className="space-y-6">
      <div
        className={`relative overflow-hidden rounded-2xl border border-border/80 bg-gradient-to-r from-card via-card ${bgGrad} p-6 shadow-sm`}
      >
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <span
                className={`inline-flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-0.5 text-xs font-semibold ${iconColor}`}
              >
                <Bot className="h-3.5 w-3.5" />
                AI Model
              </span>
              <span className="text-xs text-muted-foreground font-mono">
                Workspace / AI Model / {modelType}
              </span>
            </div>
            <h1 className="text-2xl font-bold tracking-tight text-foreground">
              {modelType} Model Manager
            </h1>
            <p className="text-sm text-muted-foreground max-w-2xl">
              Kelola model AI {modelType === "Image" ? "pembuat gambar" : "bahasa teks"} untuk
              platform Postmatic.
            </p>
          </div>
          <button
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <span className="text-[10px] text-muted-foreground mt-0.5 block">
                {modelType} AI Model &bull; {item.source}
              </span>
            </div>
          </div>
          ...
          <div className="border-t border-border/60 pt-4 space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <p className="text-[9px] text-muted-foreground">
                Kreativitas dan variasi respon model
              </p>
            </div>
            ...
            <div className="space-y-1.5 border-t border-border/40 pt-3 text-left">
              ...
              <div className="max-h-[120px] overflow-y-auto p-3 rounded-xl border border-border bg-muted/40 text-xs text-foreground leading-relaxed whitespace-pre-line font-medium shadow-inner">
                {item.preprompt || "Tidak ada kode model yang dikonfigurasi..."}
              </div>
            </div>
          </div>
