...
export function AIModelFormView({
  initialItem,
  modelType,
  onSave,
  onCancel,
    ...
    onSave(
      {
        ...
        status,
        modelCode: preprompt,
        premiumModel: initialItem?.premiumModel,
        validRatios: initialItem?.validRatios,
        imageSizes: initialItem?.imageSizes,
      },
      initialItem?.id,
    );
  };
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-lg font-bold text-foreground">
              {isEditMode ? name || `Edit ${typeLabel}` : `Buat ${typeLabel} Baru`}
            </h1>
          </div>
        </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">
              Sumber Model <span className="text-destructive">*</span>
            </label>
            <input
              type="text"
              required
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">
              Temperature <span className="text-muted-foreground font-normal">(0.0 - 2.0)</span>
            </label>
            <div className="flex items-center gap-3">
              <input
                type="range"
              ...
              />
              <span className="w-10 text-sm font-bold text-center text-foreground">
          ...
          <div className="space-y-1.5 md:col-span-2">
            <label className="text-xs font-semibold text-foreground">Kode Model</label>
            <textarea
              value={preprompt}
              onChange={(e) => setPreprompt(e.target.value)}
              rows={4}
              placeholder={modelType === "Image" ? "gpt-image-1" : "gpt-4o"}
          ...
          <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-4">
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-5 py-2 text-xs font-semibold rounded-xl bg-destructive text-destructive-foreground hover:bg-destructive/90 shadow-md transition-all disabled:pointer-events-none disabled:opacity-70"
              >
