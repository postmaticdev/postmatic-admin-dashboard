...
import { Plus, Edit3, Search, CreditCard, CheckCircle2, Clock, X, Info } from "lucide-react";
      ...
      >
        ...
        <div className="px-6 pb-6 pt-6 space-y-4">
          <div className="text-center pt-2">
            ...
            <p className="text-xs text-muted-foreground">Metode Pembayaran Platform</p>
          </div>

          <div className="border-t border-border/60 pt-4 space-y-3">
            <div className="grid grid-cols-2 gap-4 text-left">
              <div className="p-3 bg-muted/30 border border-border/50 rounded-xl">
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide block">Biaya Admin</span>
                <span className="text-sm font-semibold text-foreground mt-1 block">
                  {item.adminFeeType === "Percentage" ? `${item.adminFee}%` : formatIDR(item.adminFee)}
                </span>
              </div>
              <div className="p-3 bg-muted/30 border border-border/50 rounded-xl">
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wide block">Biaya Lain-lain</span>
                <span className="text-sm font-semibold text-foreground mt-1 block">
                  {item.otherFeeType === "Percentage" ? `${item.otherFee}%` : formatIDR(item.otherFee)}
                </span>
              </div>
          ...
          <div className="flex gap-2 pt-2 border-t border-border/40">
            ...
            >
              <Edit3 className="h-3.5 w-3.5" />Edit Metode
            </button>
            <button
              type="button"
              onClick={onClose}
...
export function PaymentTableList({ items, onCreateNew, onEdit, onToggleStatus }: PaymentTableListProps) {
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-2xl border border-border/80 bg-gradient-to-r from-card via-card to-emerald-500/5 p-6 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="space-y-1.5">
            ...
            <h1 className="text-2xl font-bold tracking-tight text-foreground">Payment Method Manager</h1>
            <p className="text-sm text-muted-foreground max-w-2xl">Kelola metode pembayaran yang tersedia di platform Postmatic.</p>
          </div>
          <button type="button" onClick={onCreateNew} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground shadow-lg shadow-primary/20 hover:bg-primary/90 active:scale-[0.98] transition-all">
            <Plus className="h-4 w-4" />Create Payment Method
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 bg-card p-4 rounded-xl border border-border/60 shadow-sm">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Cari metode pembayaran..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2 text-sm bg-background border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-all" />
        </div>
      </div>
      ...
      {selectedPayment && (
        <PaymentDetailModal
          ...
          onEdit={() => { onEdit(selectedPayment); setSelectedPayment(null); }}
        />
      )}
    </div>
  );
