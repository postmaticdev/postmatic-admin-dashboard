...
interface PaymentFormViewProps {
  ...
  onSave: (data: Omit<PaymentMethodItem, "id">, id?: string) => void;
  onCancel: () => void;
  onDelete?: (id: string) => void;
}

export function PaymentFormView({ initialItem, onSave, onCancel, onDelete }: PaymentFormViewProps) {
  ...
  const [otherFeeType, setOtherFeeType] = useState<FeeType>(initialItem?.otherFeeType || "Fixed");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) { alert("Harap isi nama payment method!"); return; }
    onSave({ name, logoUrl, adminFeeType, adminFee, otherFeeType, otherFee, status }, initialItem?.id);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-card p-4 rounded-2xl border border-border/80 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-xs font-semibold uppercase tracking-wider text-primary">{isEditMode ? "Edit Payment Method" : "Buat Payment Method Baru"}</span>
            <h1 className="text-lg font-bold text-foreground">{isEditMode ? name || "Edit Payment Method" : "Buat Payment Method Baru"}</h1>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-border bg-card p-6 shadow-sm space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">Nama Payment Method <span className="text-destructive">*</span></label>
            <input type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Misal: Bank Transfer BCA" className="w-full h-[42px] px-3.5 text-sm bg-background border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-all font-medium" />
          </div>
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">Biaya Admin ({adminFeeType === "Percentage" ? "%" : "IDR"})</label>
            <input
              type="text"
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">Tipe Biaya Lainnya</label>
            <select
              value={otherFeeType}
              onChange={(e) => {
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-foreground">Biaya Lainnya ({otherFeeType === "Percentage" ? "%" : "IDR"})</label>
            <input
              type="text"
              ...
              onChange={(e) => {
                ...
                if (otherFeeType === "Fixed") {
                  setOtherFee(parseNumberString(val));
                } else {
                  const num = Number(val.replace(/\D/g, ""));
                  setOtherFee(num > 100 ? 100 : num);
                }
              }}
              placeholder="0"
              className="w-full h-[42px] px-3.5 text-sm bg-background border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-all font-medium"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-card border border-border rounded-2xl p-6 w-full max-w-md shadow-2xl space-y-4">
            <div className="flex items-center gap-3 text-destructive">
              ...
              <div>
                <h3 className="font-bold text-base text-foreground">Konfirmasi Hapus Payment Method</h3>
                <p className="text-xs text-muted-foreground">Tindakan ini tidak dapat dibatalkan</p>
              </div>
            </div>
            <p className="text-sm text-muted-foreground">Apakah Anda yakin ingin menghapus <strong className="text-foreground">"{initialItem.name}"</strong>?</p>
